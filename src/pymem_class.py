...
class PyMem:
    ...
    def dump_and_save_memory(filename, memsize = None):
        ...
        print("Creating Volatility-compatible raw memory dump")
        ...
        try:
            ...
            with open(filename + ".raw", "wb") as f:
                ...
                print(f"\nRaw dump completed. Total dumped: {total_dumped / 1024 / 1024:.2f} MB")
                
            # Create metadata file for Volatility
            PyMem.create_volatility_metadata(filename, memory_runs, memsize)
                
        finally:
            win32file.CloseHandle(device_handle)

    @staticmethod
    def create_volatility_metadata(filename, memory_runs, memsize):
        """Create metadata file for Volatility analysis"""
        import json
        import datetime
        
        metadata = {
            "format": "raw",
            "architecture": "x64" if ctypes.sizeof(ctypes.c_voidp) == 8 else "x86",
            "platform": "windows",
            "memory_runs": memory_runs,
            "total_memory": memsize,
            "dump_time": datetime.datetime.now().isoformat(),
            "volatility_compatible": True,
            "notes": "Generated by PyMem for Volatility analysis"
        }
        ...
        # Create Volatility info file
        with open(filename + "_volatility_info.txt", "w") as f:
            f.write("Volatility Memory Dump Information\n")
            f.write("=" * 40 + "\n\n")
            f.write(f"File: {filename}.raw\n")
            f.write(f"Format: Raw memory dump\n")
            f.write(f"Architecture: {metadata['architecture']}\n")
            f.write(f"Platform: {metadata['platform']}\n")
            f.write(f"Total Memory: {memsize / 1024 / 1024 / 1024:.2f} GB\n")
            f.write(f"Dump Time: {metadata['dump_time']}\n")
            f.write(f"Memory Runs: {len(memory_runs)}\n\n")
            
            f.write("Memory Runs:\n")
            f.write("-" * 20 + "\n")
            for i, (start, length) in enumerate(memory_runs):
                f.write(f"Run {i+1}: 0x{start:016x} - 0x{start + length:016x} ({length/1024/1024:.2f} MB)\n")
            
            f.write("\nVolatility Commands:\n")
            f.write("-" * 20 + "\n")
            f.write(f"python vol.py -f {filename}.raw imageinfo\n")
            f.write(f"python vol.py -f {filename}.raw --profile=Win10x64 pslist\n")
            f.write(f"python vol.py -f {filename}.raw --profile=Win10x64 pstree\n")
            f.write(f"python vol.py -f {filename}.raw --profile=Win10x64 cmdline\n")
            f.write(f"python vol.py -f {filename}.raw --profile=Win10x64 filescan\n")
            f.write(f"python vol.py -f {filename}.raw --profile=Win10x64 netscan\n")
        
        print(f"Metadata files created:")
        print(f"  - {filename}_metadata.json")
        print(f"  - {filename}_volatility_info.txt")
    ...
    @staticmethod
    def dump_volatility_compatible(filename="volatility_dump"):
        """Create Volatility-compatible memory dump with proper formatting"""
        print("=== VOLATILITY COMPATIBLE MEMORY DUMP ===")
        print("Creating raw memory dump optimized for Volatility analysis...")
        PyMem.dump_and_save_memory(filename)
        print("\nVolatility dump completed!")
        print(f"Files created:")
        print(f"  - {filename}.raw (Raw memory dump)")
        print(f"  - {filename}_metadata.json (Metadata)")
        print(f"  - {filename}_volatility_info.txt (Volatility commands)")
        print(f"\nTo analyze with Volatility:")
        print(f"  python vol.py -f {filename}.raw imageinfo")
        print(f"  python vol.py -f {filename}.raw --profile=Win10x64 pslist")